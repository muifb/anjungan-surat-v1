...
class Validation extends Database
{
  ...
  public function is_unique(array $data, string $field, string $table, string $column): bool
  {
    ...
    $query = $this->get([$column => $data[$field]])->fetch(PDO::FETCH_ASSOC);
    if ($query) {
      return false;
    }
  ...
  public function is_trim(array $data, string $field): bool
  {
    if (!isset($data[$field])) {
      return true;
    }

    if ($data[$field] == trim($data[$field]) && str_contains($data[$field], ' ')) {
